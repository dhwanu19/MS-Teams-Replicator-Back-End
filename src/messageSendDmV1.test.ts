import { authRegisterV1 } from './authRequests';
import { dmCreateV1 } from './dmRequests';
import { messageSendDmV1 } from './messageRequests';
import { clearV1 } from './otherRequests';

beforeEach(() => {
  clearV1();
});

// Success cases:
// - 1 character edge case
// - 1000 character edge case
// - normal message length

describe('Success Tests', () => {
  test('Message of length 1 character => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const message = '1';
    expect(messageSendDmV1(creator.token, dmId, message)).toStrictEqual({ messageId: expect.any(Number) });
  });
  test('Message of length 1000 characters => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const message = '';
    expect(messageSendDmV1(creator.token, dmId, message)).toStrictEqual({ messageId: expect.any(Number) });
  });
  test('Message of normal length => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    dmCreateV1(creator.token, []);
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const message = 'When the washing machine is broken XDXDXD';
    expect(messageSendDmV1(creator.token, dmId, message)).toStrictEqual({ messageId: expect.any(Number) });
  });
});

// Errorr cases:
// - token is invalid
// - dmId is not valid
// - lenth of message is less than 1 or over 1000 characters
// - dmId is valid but authorised user is not part of the channel

describe('Error Tests', () => {
  test('Invalid token => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const invalidToken = creator.token + '1';
    expect(messageSendDmV1(invalidToken, dmId, 'Message!')).toStrictEqual({ errorCode: 403 });
  });
  test('Invalid dmId => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const invaliddmId = 1234;
    expect(messageSendDmV1(creator.token, invaliddmId, 'Message!')).toStrictEqual({ errorCode: 400 });
  });
  test('Message length < 1 => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    expect(messageSendDmV1(creator.token, dmId, '')).toStrictEqual({ errorCode: 400 });
  });
  test('Message length > 1000 => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const message = '';
    expect(messageSendDmV1(creator.token, dmId, message)).toStrictEqual({ errorCode: 400 });
  });
  test('Authorised user not part of dm => error', () => {
    const creator = authRegisterV1('test@gmail.com', 'Password', 'Bill', 'Gates');
    const otherUser = authRegisterV1('other@gmail.com', 'Password', 'Jim', 'Halpert');
    const dmId = dmCreateV1(creator.token, [otherUser.authUserId]).dmId;
    const invalidToken = authRegisterV1('timmy@gmail.com', 'Password', 'Bob', 'Thebuilder').token;
    expect(messageSendDmV1(invalidToken, dmId, 'Message!')).toStrictEqual({ errorCode: 403 });
  });
});
